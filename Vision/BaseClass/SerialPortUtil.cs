...
namespace Vision.BaseClass
{
    ...
    public class SerialPortUtil
    {
        ...
        /// 结束符比特
        /// </summary>
        public byte EndByte = 0x23;//string End = "#",结束符;
        public string EndString = "#";//string End = 回车符
        /// <summary>
        /// 完整协议的记录处理事件
        /// </summary>
      //  public event DataReceivedEventHandler DataReceived;
        ...
        public event PushEventHandler PushSerial; //消息推送
            ...
            StopBits sBits)
        {
            ...
            {
                PortName = name, // 指定正确端口
                BaudRate = (int)baud,
                Parity = par,
                DataBits = (int)dBits,
                StopBits = sBits,
                Handshake = Handshake.None, // 禁用流控制
                WriteTimeout = 2000 // 设置超时
            };
        ...
        void comPort_DataReceived(object sender, SerialDataReceivedEventArgs e)
        {
            ...
            if (num > 0)
            {
                //ReceivedMsg = comPort.ReadTo(EndString).Trim(); //读取到回车符号
                 ReceivedMsg = comPort.ReadExisting();
                 comPort.DiscardInBuffer();
                 if (ReceivedMsg != null)
                 {                     
                     PushEvent();//将接收到的消息推送至FramMain
                 }                   
            }
        ...
        public void WriteData(string msg)
        {
            ...
            {
                // 记录或处理异常
                LogUtil.LogError($"SerialPortError: {ex}");
            }
        }
