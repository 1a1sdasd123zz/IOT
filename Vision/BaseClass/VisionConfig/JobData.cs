...
namespace Vision.BaseClass.VisionConfig
{
    public class JobData
    {
        ...
        public string Name = "";
        public BaseInfo mCameraInfo = new();
        ...
        public readonly MyDictionaryEx<ImageDisplay> mImageDisplays = new();
        public ModuleData<Terminal, Module.Algorithm.Info> mAlgModuleData = new();

        //public MyDictionaryEx<PositionInfo> mLocInfo = new();
        public VisionParameters mVisionParameters = new();
        ...
        public string VisionParametersPath;
        ...
        public JobData(int jobID, string jobName, SystemConfigData systemConfigData)
        {
            ...
            VisionParametersPath = mSystemConfigData.JobPath + "VisionParameters.xml";
        ...
        public void InitParam()
        {
            ...
            {
                if (File.Exists(VisionParametersPath))
                {
                    try
                    {
                        mVisionParameters = XmlHelper.ReadXML<VisionParameters>(VisionParametersPath);
                    }
                    catch (Exception ex)
                    {
                        LogUtil.LogError("解析本地" + VisionParametersPath + "文件失败，异常信息：" + ex);
                    }
                }
            }
            else
        ...
        private void InitAlgTool()
        {
            ...
            var stations = (EnumStation[])Enum.GetValues(typeof(EnumStation));
            foreach (var item in stations)
            {
                try
                {
                    var key = item.GetDescription();
                    var tb = (CogToolBlock)CogSerializer.LoadObjectFromFile(mSystemConfigData.AlgMoudlePath + key + ".vpp");
                    mTools.Add(key, tb);
                }
                catch (Exception ex)
                {
                    LogUtil.LogError($"加载vpp异常，" + ex); ;
                }
            }
            LogUtil.Log($"工具加载完成！");
        }
