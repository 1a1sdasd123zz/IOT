...
namespace Vision.Hardware.C_2DGigeLineScan.iRAYPLE
{
    internal class DaHua_GigeLineScan
    {
        ...
        private unsafe void StreamGrabber_ImageGrabbed(object sender, GrabbedEventArgs e)
        {
            try
            {
                ...
                else if (CvtGvspPixelFormatType(grebRawDate.PixelFmt) == 3)
                {
                    ...
                    int nDesDataSize = 0;
                    int ret = IMGCNV_ConvertToBGR24_Ex(rawPtr, ref sOpenParam, colorPtr, ref nDesDataSize, IMGCNV_EBayerDemosaic.IMGCNV_DEMOSAIC_NEAREST_NEIGHBOR);
                    pTemp = colorPtr;
                    if (ret != 0)
                    {
                        LogPush("图像转码出错！");
                    ...
                    {
                        byte[] colorBGRData = new byte[datdSize];
                        byte* pBufForSaveImage = (byte*)(void*)pTemp;
                        uint nSupWidth = (uint)(grebRawDate.Width + 3) & 0xFFFFFFFCu;
                        for (int nRow = 0; nRow < grebRawDate.Height; nRow++)
                        {
                            for (int col = 0; col < grebRawDate.Width; col++)
                            {
                                colorBGRData[nRow * nSupWidth + col] = pBufForSaveImage[nRow * grebRawDate.Width * 3 + (3 * col + 2)];
                                colorBGRData[grebRawDate.Width * grebRawDate.Height + nRow * nSupWidth + col] = pBufForSaveImage[nRow * grebRawDate.Width * 3 + (3 * col + 1)];
                                colorBGRData[grebRawDate.Width * grebRawDate.Height * 2 + nRow * nSupWidth + col] = pBufForSaveImage[nRow * grebRawDate.Width * 3 + 3 * col];
                            }
                        }
                        imagedata.Data = colorBGRData;
